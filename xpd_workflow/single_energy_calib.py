...
def find_peaks(intensity, sides=6):
    ...
    criteria = intensity[preliminary_peaks2] >= 2 * intensity[preliminary_peaks2 + sides]
    criteria *= intensity[preliminary_peaks2] >= 2 * intensity[preliminary_peaks2 - sides]
...
def calibrate_energy(intensity, ttheta, calibration_file, sides=12):
    ...
    # d spacing
    d_spacings = d_spacings[:len(fitted_peak_centers)]
    for peak, d in zip(fitted_peak_centers, d_spacings):
        lam = lamda_from_bragg(peak, d, 1)
