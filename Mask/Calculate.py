...
def corrections(xray_image, solid=False, pol=True, geometry_object=None,
...
def statistics1d(img, max=None, step=None, position_matrix=None,
                 statistic=None):
    """
...
def write_mask(xray_image, geometry_object, mask=None, initial_m=None,
    ...
    modImage = corrections(xray_image, solid=False, pol=True,
                           geometry_object=a, polarization_factor=.95)

    # Define a bunch of statistics on the unmasked image
    rawmean = statistics1d(modImage, max=maxr, step=1,
                           position_matrix=roundR, statistic='mean')

    rawstd = statistics1d(modImage, max=maxr, step=1,
                          position_matrix=roundR, statistic=np.std)

    pixels_per_bin = statistics1d(modImage, max=maxr, step=1,
    ...
    while True:
        ...
        newmean = statistics1d(modImage, max=np.amax(q_matrix), step=resolution,
                               position_matrix=q_matrix * iMask,
                               statistic='mean')
        q_rawmean = statistics1d(modImage, max=np.amax(q_matrix),
                                 step=resolution,
                                 position_matrix=q_matrix, statistic='mean')
